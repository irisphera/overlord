...
class CliCharacterizationTests(unittest.TestCase):
    def test_help_aliases_print_current_usage_without_starting_engine(self) -> None:
        for argument in ("help", "-h", "--help"):
            with self.subTest(argument=argument), launcher_workspace() as workspace:
                result = run_launcher(workspace, argument)
    ...
    def test_invalid_cli_inputs_are_rejected(self) -> None:
        cases = (
            (
            ...
            (
                "removed_lms_model_option",
                ("--lms-model", "sentinel-model"),
                "Error: unknown option '--lms-model'\nRun 'overlord help' for usage.\n",
            ),
            (
                "unknown_command",
                ("status",),
        ...
        for name, args, expected_stderr in cases:
