...
from overlord_py.web_scripts import REQUEST_RESTART_IF_MODE_CHANGED_SCRIPT, REQUEST_RESTART_IF_PLUGIN_ENV_MISSING_SCRIPT, RESTART_OPENCODE_WEB_SCRIPT
...
def request_opencode_web_restart_if_mode_changed(engine: EngineRunner, paths: WorkspacePaths, desired_mode: str, restart: RestartState, *, env: Mapping[str, str]) -> tuple[str, ...]:
    if restart.required:
        return ()
    ...
    if result.returncode == 0:
        return ()
    restart.request()
    return (f"Restarting existing OpenCode web server because Headroom mode changed or its mode marker is missing/stale in {paths.identity.container_name}...",)
...
def request_opencode_web_restart_if_plugin_env_missing(
    ...
    *,
    ...
    if result.returncode == 0:
        return ()
    restart.request()
    return (f"Restarting existing OpenCode web server because its HOME/XDG/CodeGraph/MCP credential environment is not canonical in {paths.identity.container_name}...",)


def restart_opencode_web_if_needed(engine: EngineRunner, paths: WorkspacePaths, restart: RestartState, *, env: Mapping[str, str]) -> tuple[str, ...]:
    ...
    return (f"Restarting OpenCode web server so package/config repairs take effect in {paths.identity.container_name}...",)
