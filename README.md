...
| --- | --- | --- |
| Prime Agent — Astra | `gpt-6-astra` | `off` → `none`, `low`, `medium`, `high`, `xhigh`, `max` |
| Codex — normal work | `gpt-5.6-luna` | `max` |
| Codex — high-brain profile | `gpt-6-astra` | `medium` |

- **Prime Agent:** Astra explicitly maps every supported effort in both the custom model and its override. `off` sends literal Azure `reasoning.effort: "none"`; `xhigh` and `max` pass through unchanged. The unsupported `minimal` selector is disabled. Re-running setup repairs missing or stale mappings without changing the selected default model or reasoning level.
- **Astra context window:** Setup configures `gpt-6-astra` for 272,000 tokens in Prime Agent and Codex's high-brain profile. Re-running setup updates existing managed Astra limits; other models keep their existing policy.
...
- `AZURE_OPENAI_BASE_URL` takes precedence over `AZURE_OPENAI_RESOURCE_NAME`. Codex resolves both model names through `AZURE_OPENAI_DEPLOYMENT_NAME_MAP`; Prime's Azure adapter resolves that map at runtime. API keys stay in the environment, not in the generated configuration.
...
- Prime sessions, configuration, skills, and databases live in the workspace's `.overlord/prime-agent-data` directory and survive `fresh`/`purge`. Missing authored configuration and skills seed from image defaults without overwriting existing state or copying image sessions/auth databases.
- Verified legacy basename containers can be adopted. Containers with extra mounts (including old OMP state, host SSH/config, or engine sockets), a changed socket opt-in, or a missing entrypoint contract are stopped and recreated before attachment. Missing required Prime/zsh/workspace mounts still refuse deletion. Existing host `.overlord/omp-agent-data` is left untouched but is no longer mounted or managed; unmounted OMP state inside old containers is no longer rescued. Export any needed container-only OMP data before migration. Use `overlord purge` to rebuild without OMP.
