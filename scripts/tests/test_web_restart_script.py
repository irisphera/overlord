...
class WebRestartScriptTests(unittest.TestCase):
    def test_unrelated_live_pid_is_not_signaled(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            result = self._run_restart_script(pid_file)
    ...
    def test_matching_process_surviving_sigterm_fails_and_retains_pid_file(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            state_dir = Path(temporary_directory)
            pid_file = state_dir / "opencode.pid"
    ...
    def test_process_status_infrastructure_error_fails_without_signaling_or_clearing_pid(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            state_dir = Path(temporary_directory)
            pid_file = state_dir / "opencode.pid"
            ...
            result = self._run_restart_script(pid_file, env=env)

            self.assertNotEqual(result.returncode, 0)
            self.assertIsNone(process.poll())
            self.assertTrue(pid_file.exists())

    def test_dead_pid_clears_pid_file(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            state_dir = Path(temporary_directory)
            pid_file = state_dir / "opencode.pid"
    ...
    def test_nonnumeric_pid_clears_pid_file(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            state_dir = Path(temporary_directory)
            pid_file = state_dir / "opencode.pid"
    ...
    def _run_restart_script(
        ...
        *,
        ...
        return subprocess.run(
            ["sh", "-s", "--", str(pid_file), "0.0.0.0", "4090"],
            env=env,
